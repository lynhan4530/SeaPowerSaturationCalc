...
**Update (post-launch):** a **time-based radar-horizon model** now scales each
system's shots by how much of a full channel salvo fits in the horizon-limited
engagement window (`reach`, driven by attacker altitude + speed and the scenario
radar height) — see CLAUDE.md "Radar Horizon SAM Range Capping" and deviation #6.
`altitudeFt == null` ⇒ `reach = 1`, so this design's math is unchanged for
high-altitude raids.
